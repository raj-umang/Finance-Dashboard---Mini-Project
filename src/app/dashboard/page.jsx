...
const Dashboard = async() => {
  ...
  const expense_summary = expenses.reduce((total,item)=>(item.transaction_amount+total),0)
  const income_summary = income.reduce((total,item)=>(item.transaction_amount+total),0)
  const investment_summary = investment.reduce((total,item)=>(item.transaction_amount+total),0)
    ...
    <div className="flex gap-5">
      <div className="basis-full flex flex-col g-5">
        <div className='flex g-5 justify-between my-2'>
          ...
          <Card data={investment_summary} type={'investment'} title={'Investment'}/>
          <PieChartComponent expenses={expense_summary} investment={investment_summary} income={income_summary}/>
        </div>
        <Transaction />
